...
На очных парах будут устно объявляться теоретические вопросы, ответ на которые не следует из материалов лекции напрямую. Первый приславший верный ответ на электронную почту преподавателя - получает баллы.

# Как считаем баллы БРС
Вот так:

SCORE = (if есть_верифицированный_акк_на_каггле then 5 else 0) + MIN(35, ULEARN% / 2) + CONTEST_1 + CONTEST_2 + THEORY
